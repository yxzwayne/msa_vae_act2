...
class Base_VAE:
    ...
    def load(self, name):
        ...
        self.vae = keras.models.load_model(
            name + "_vae.k",
            compile=False,
            custom_objects={"_sampling": self._sampling, "_vae_loss": self._vae_loss},
        )
        self._extract_layers()
    ...
    def train(
        ...
        if early_patience is not None:
            callbacks.append(EarlyStopping(monitor="val_loss", patience=early_patience))
        if name is not None:
...
class Deep_VAE(Base_VAE):
    ...
    def _enc_dec_layers(self, L, q, latent_dim, batch_size, depth):
        ...
        enc_layers = [
            Dense(Lq // (2 ** (d + 1)), activation="elu") for d in range(depth)
        ]
        dec_layers = [
            Dense(Lq // (2 ** (d + 1)), activation="elu") for d in range(depth)[::-1]
        ]
...
cdict = {
    "red": ((0.0, 0.0, 0.8), (1.0, 0.0, 0.0)),
    "green": ((0.0, 0.0, 0.95), (1.0, 0.25, 1.0)),
    "blue": ((0.0, 0.0, 1.0), (1.0, 0.5, 1.0)),
}
...
def main_plot_latent(name, args):
    ...
    if vae.latent_dim == 1:
        ...
        for p, zi in zip(psm, z):
            ...
            im = ax2.imshow(p.T, cmap="gray_r", interpolation="nearest", animated=True)
            artists.append([im, zpt])
            # print("".join(ALPHA[c] for c in np.argmax(p, axis=1)))
        ani = animation.ArtistAnimation(
            fig, artists, interval=40, blit=True, repeat_delay=1000
        )
    ...
    X, Y = np.meshgrid(s, s)
    red = np.broadcast_to(np.array([1.0, 0, 0, 1]), (len(s), len(s), 4)).copy()
    ...
    for z1 in range(latent_dim):
        ...
        for z2 in range(z1 + 1, latent_dim):
            ...
            plt.hist2d(
                m[:, z2], m[:, z1], bins=np.linspace(-r, r, 50), cmap=DarkBlue, cmin=1
            )

            nn = norm.pdf(X, m[0][z2], st[0][z2]) * norm.pdf(Y, m[0][z1], st[0][z1])
            nn = nn / np.max(nn) / 1.5
            red[:, :, 3] = nn
